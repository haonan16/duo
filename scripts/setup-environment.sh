...
configure_shell() {
    ...
    for rc in "${rc_files[@]}"; do
        if grep -qF '/.duo/duo.sh' "$rc" 2>/dev/null; then
...
add_to_shell_rc() {
    ...
    # Always install/update the wrapper first
    install_wrapper

    # Check if the stable path is already configured
    if grep -qF '/.duo/duo.sh' "$rc_file" 2>/dev/null; then
        echo "Already configured in $rc_file"
        return 0
    fi

    # Migrate: replace old versioned duo.sh source lines with the stable path
    if grep -qF "duo.sh" "$rc_file" 2>/dev/null; then
        # Remove old source line(s) referencing duo.sh (versioned or otherwise)
        sed -i.duo-bak '/[Ss]ource.*duo\.sh/d' "$rc_file"
        # Also remove the comment marker if it's now orphaned
        sed -i '/^# Duo monitor helper$/d' "$rc_file"
        rm -f "${rc_file}.duo-bak"
        echo "Migrated old duo.sh source line in $rc_file"
    fi

    printf '\n# Duo monitor helper\nsource "$HOME/.duo/duo.sh"\n' >> "$rc_file"
