...
# - Summary files outside .duo/rlcr/
# - Goal tracker after Round 0
# - PR loop state files (.duo/pr-loop/)
...
# If not a summary file, not a finalize summary, and not in .duo/rlcr, allow normally
if [[ "$IS_SUMMARY_FILE" == "false" ]] && [[ "$IS_FINALIZE_SUMMARY" == "false" ]] && [[ "$IN_DUO_LOOP_DIR" == "false" ]]; then
    exit 0
fi

# For state.md, finalize-state.md, goal-tracker.md, and plan.md in .duo/rlcr, we need further validation
# For other files in .duo/rlcr that aren't summaries, allow them
FILENAME=$(basename "$FILE_PATH")
IS_PLAN_BACKUP=$([[ "$FILENAME" == "plan.md" ]] && echo "true" || echo "false")
if [[ "$IN_DUO_LOOP_DIR" == "true" ]] && [[ "$IS_SUMMARY_FILE" == "false" ]] && [[ "$IS_FINALIZE_SUMMARY" == "false" ]]; then
...
# This must be checked BEFORE the "summary files outside .duo/rlcr" check

if [[ "$IS_FINALIZE_SUMMARY" == "true" ]] && [[ "$IN_DUO_LOOP_DIR" == "true" ]]; then
...
# ========================================
# Block Summary Files Outside .duo/rlcr
# ========================================

if [[ "$IS_SUMMARY_FILE" == "true" ]] && [[ "$IN_DUO_LOOP_DIR" == "false" ]]; then
...
CLAUDE_FILENAME=$(echo "$FILE_PATH" | sed -n 's|.*\.duo/rlcr/[^/]*/\(.*\)$|\1|p')
