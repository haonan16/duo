...
# Check if a path is inside .duo/rlcr directory
is_in_duo_loop_dir() {
    local path="$1"
    echo "$path" | grep -q '\.duo/rlcr/'
}
...
# Check if a path is inside .duo/pr-loop directory
is_in_pr_loop_dir() {
    local path="$1"
    echo "$path" | grep -q '\.duo/pr-loop/'
}

# Check if a path is inside any loop directory (RLCR or PR loop)
is_in_any_loop_dir() {
    local path="$1"
    is_in_duo_loop_dir "$path" || is_in_pr_loop_dir "$path"
}
...
validate_pr_resolve_round() {
    ...
    local pr_loop_base_dir="$project_root/.duo/pr-loop"
...
You cannot write to round-*-pr-comment.md or round-*-prompt.md files in .duo/pr-loop/.
...
# Check if a git add command would add .duo files to version control
# Usage: git_adds_duo "$command_lower"
# Returns 0 if the command would add .duo files, 1 otherwise
#
...
# - Chained commands: cd repo && git add .duo
# - Shell operators: ;, &&, ||, |
#
# Blocks:
# - git add .duo or git add .duo/
# - git add .duo/* or git add .duo/**
# - git add -f .duo* (force add)
...
# - git add -A or git add --all (when .duo exists)
# - git add . or git add * (when .duo exists and not gitignored)
#
git_adds_duo() {
    local cmd="$1"

    # Split command on shell operators and check each segment
    # This handles chained commands like: cd repo && git add .duo
    local segments
    ...
    while IFS= read -r segment; do
        ...
        # This handles: git add ".duo", git add '.duo'
        local add_args_normalized
        add_args_normalized=$(echo "$add_args" | sed "s/[\"']//g")

        # Check for direct .duo reference (blocked regardless of other flags)
        # Handles: .duo, ./.duo, path/to/.duo, ".duo", '.duo'
        # Pattern matches .duo at start, after space, after / or ./ AND followed by end, /, or space
        # This avoids over-blocking .duoconfig or .duo-backup
        if echo "$add_args_normalized" | grep -qE '(^|[[:space:]]|/)\.duo($|/|[[:space:]])'; then
            return 0
        fi
        ...
        # Check if .duo exists - needed for non-force blocking
        if [[ ! -d ".duo" ]]; then
            continue
        fi

        # git add -A/--all when .duo exists
        ...
        # git add . or git add * when .duo exists and not gitignored
        # Only block if .duo is NOT protected by gitignore
        if [[ "$has_broad_scope" == "true" ]]; then
            if ! git check-ignore -q .duo 2>/dev/null; then
                return 0
            fi
        fi
    done <<< "$segments"
...
# Standard message for blocking git add .duo commands
# Usage: git_add_duo_blocked_message
git_add_duo_blocked_message() {
    local fallback="# Git Add Blocked: .duo Protection
...
IMPORTANT: The commit message must NOT contain the literal string \".duo\" to avoid triggering this protection."
    ...
    load_and_render_safe "$TEMPLATE_DIR" "block/git-add-duo.md" "$fallback"
}
